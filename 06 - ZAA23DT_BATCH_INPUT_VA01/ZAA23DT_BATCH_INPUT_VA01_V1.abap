*&---------------------------------------------------------------------*
*& Report  ZAA23DT_BATCH_INPUT_VA01
*&---------------------------------------------------------------------*
...
FORM format_file
  ...
  LOOP AT us_t_file INTO lv_string.
    ...
    ELSEIF lv_string(1) EQ 'D'.
      ...
      CLEAR:
        le_pedidos_file-material,
        le_pedidos_file-cantidad,
        le_pedidos_file-centro.
      ...
      le_pedidos_file-cantidad = lv_cantidad_char.

      CALL FUNCTION 'CONVERSION_EXIT_MATN1_INPUT'
        EXPORTING
          input        = le_pedidos_file-material
        IMPORTING
          output       = le_pedidos_file-material
        EXCEPTIONS
          length_error = 1
          OTHERS       = 2.
...
FORM BATCH_INPUT_VA01
  ...
  DATA:
    le_pedidos_file     TYPE ty_pedidos_file,
    le_pedidos_file_aux TYPE ty_pedidos_file,
    lt_bdcdata          TYPE tyt_bdcdata, " tipo batch input
  ...
  "si no ponngo esto muchas veces falla el batch input"
  le_options-dismode  = 'N'. " A(Append) -> Mostrar BI / N -> No Append  / E -> Error if exist
  le_options-updmode  = 'S'.
  le_options-racommit = abap_true.

  LOOP AT us_t_pedidos_file INTO le_pedidos_file.

    le_pedidos_file_aux = le_pedidos_file.
    ...
    AT NEW pedido.
      ...
      PERFORM bdc_field USING 'VBAK-AUART'
                              le_pedidos_file_aux-clase_pedido
                     CHANGING lt_bdcdata.

      PERFORM bdc_field USING 'VBAK-VKORG'
                              le_pedidos_file_aux-org_ventas
                     CHANGING lt_bdcdata.

      PERFORM bdc_field USING 'VBAK-VTWEG'
                              le_pedidos_file_aux-canal_distr
                     CHANGING lt_bdcdata.

      PERFORM bdc_field USING 'VBAK-SPART'
                              le_pedidos_file_aux-sector
                     CHANGING lt_bdcdata.
      ...
      PERFORM bdc_field USING 'VBKD-BSTKD'
                              le_pedidos_file_aux-pedido_cliente
                     CHANGING lt_bdcdata.

      PERFORM bdc_field USING 'KUAGV-KUNNR'
                              le_pedidos_file_aux-solicitante
                     CHANGING lt_bdcdata.
    ...
    PERFORM bdc_field USING lv_field
                            le_pedidos_file_aux-material
                   CHANGING lt_bdcdata.

    CONCATENATE 'RV45A-KWMENG(' lv_posicion ')' INTO lv_field.

    WRITE le_pedidos_file_aux-cantidad TO lv_cantidad.
    CONDENSE lv_cantidad. "numero a la izquierda"

    PERFORM bdc_field USING lv_field
                            lv_cantidad
                   CHANGING lt_bdcdata.

    IF le_pedidos_file_aux-centro IS NOT INITIAL.

      CONCATENATE 'VBAP-WERKS(' lv_posicion ')' INTO lv_field.

      PERFORM bdc_field USING lv_field
                              le_pedidos_file_aux-centro
                     CHANGING lt_bdcdata.
    ...
    AT END OF pedido.
      ...
      CALL TRANSACTION 'VA01' "los tres parametrosque mas usamos"
                 USING lt_bdcdata
          OPTIONS FROM le_options
         MESSAGES INTO lt_messages. "devuelve todos los mensajes que veo, ya sea de error, de informacion o algo"
